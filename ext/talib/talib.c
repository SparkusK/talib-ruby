...
#define IN_CNT                          7  // allow up to 7 arrays of
                                           // input
#define OUT_CNT                         3  // allow up to 3 arrays of
                                           // outputs
...
static double* FLT2DBL(double **dest, VALUE in_array)
{
  ...
  *dest = calloc(RARRAY_LEN(in_array), sizeof(double));
...
static VALUE ta_func_setup_in_price(VALUE self, VALUE param_index, VALUE in_open, VALUE in_high, VALUE in_low, VALUE in_close, VALUE in_volume, VALUE in_oi)
{
  ...
  ret_code = TA_SetInputParamPricePtr(param_holder->p,
                                      FIX2INT(param_index),
